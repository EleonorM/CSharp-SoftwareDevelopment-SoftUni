namespace _01.AvlTree
{
        ...
        where T : IComparable<T>
    {
        ...
        public List<T> FindInRange(T from, T to)
        {
            var node = root;
            var result = new List<T>();
            InOrderDfs(node, from, to, result);
        ...
        private void InOrderDfs(Node<T> node, T from, T to, List<T> numbers)
        {
            if (node.LeftChild != null && node.LeftChild.Value.CompareTo(from) >= 0)
            {
                InOrderDfs(node.LeftChild, from, to, numbers);
            ...
            {
                InOrderDfs(node.RightChild, from, to, numbers);
            }
        }
    }
}
